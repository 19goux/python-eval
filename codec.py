...
class TreeBuilder:
    ...
    def tree(self):
        ...
        while len(clefs)>1:
            ...
            dico[a+b]=dico[a]+dico[b] #on les fusionne et on rajoute une instance dans le dictionnaire
...
class Codec:
    ...
    def decode(self, code):
        inv_d = {v: k for k, v in self.binary_tree.items()}
        inverted_on = []
        result = ''
        for i in code:
            inverted_on.append(i)
        inverted = inverted_on[::-1] #On obtient une liste des caractères de code commencant par la fin
        cha = inverted.pop()
